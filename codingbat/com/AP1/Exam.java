...
public class Exam {
   ...
   * if they are the same. Order first by the string names, and then by the id numbers
   * if the names are the same.
   * Note: with Strings str1.compareTo(str2) returns an int value
  ...
  public int userCompare(String aName, int aId, String bName, int bId) {
    //DeOut.disp("A:"+aName+" B: "+bName+" compareTo= " + aName.compareTo(bName));
   ...
   * return the number of "big" steps for a walk starting at the start index
  ...
  public int bigHeights(int[] heights, int start, int end) {
    ...
    for(int i=0;i<count;i++){
      int diff = Math.abs(heights[start+i]-heights[start+i+1]);
      if(diff>=5) sum++;
    }
    return sum;
  }
